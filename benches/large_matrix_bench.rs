...
use magnus::{magnus_spgemm, magnus_spgemm_parallel, reference_spgemm, MagnusConfig, SparseMatrixCSR};
...
fn generate_bench_matrix(
    ...
    for _ in 0..rows {
        let row_nnz = rng.gen_range((nnz_per_row / 2)..=(nnz_per_row * 3 / 2)).min(cols);
...
fn bench_quick_sanity(c: &mut Criterion) {
    ...
    group.bench_function("magnus_1k", |b| {
        ...
        b.iter(|| {
            magnus_spgemm(black_box(&small), black_box(&small), &config)
        });
    });
    
    group.bench_function("reference_1k", |b| {
        b.iter(|| {
            reference_spgemm(black_box(&small), black_box(&small))
        });
    });
    ...
    group.bench_function("magnus_parallel_5k", |b| {
        let config = MagnusConfig::default();
        b.iter(|| {
            magnus_spgemm_parallel(black_box(&medium), black_box(&medium), &config)
        });
    });
...
fn bench_large_matrices(c: &mut Criterion) {
    ...
    for (name, rows, cols, nnz_per_row) in test_cases {
        ...
        group.bench_with_input(
            BenchmarkId::new("magnus_parallel", format!("{}_{:.1}M", name, total_nnz as f64 / 1e6)),
            &matrix,
            |b, m| {
                let config = MagnusConfig::default();
                b.iter(|| {
                    magnus_spgemm_parallel(black_box(m), black_box(m), &config)
                });
            },
        );
        ...
        if rows <= 20_000 {
            group.bench_with_input(
                BenchmarkId::new("magnus_serial", format!("{}_{:.1}M", name, total_nnz as f64 / 1e6)),
                &matrix,
                |b, m| {
                    let config = MagnusConfig::default();
                    b.iter(|| {
                        magnus_spgemm(black_box(m), black_box(m), &config)
                    });
                },
            );
        }
...
fn bench_scaling_study(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        group.bench_with_input(
            BenchmarkId::new("scaling", format!("{}x{}_{:.1}M", size, size, total_nnz as f64 / 1e6)),
            &matrix,
            |b, m| {
                let config = MagnusConfig::default();
                b.iter(|| {
...
fn bench_density_study(c: &mut Criterion) {
    ...
    for nnz_per_row in densities {
        ...
        group.bench_with_input(
            BenchmarkId::new("density", format!("nnz{}_{:.3}%", nnz_per_row, density * 100.0)),
            &matrix,
            |b, m| {
                let config = MagnusConfig::default();
                b.iter(|| {
