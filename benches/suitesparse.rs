...
fn bench_suitesparse(c: &mut Criterion) {
    ...
    if !suitesparse_dir.exists() {
        eprintln!("SuiteSparse directory not found. Create ./data/suitesparse and add matrices there.");
        return;
    }
            ...
            Ok(a) => {
                ...
                if let Some(row_idx) = select_representative_row(&a, RowType::Medium) {
                    group.bench_with_input(
                        ...
                        |bench, _| {
                            bench.iter(|| {
                                let (cols, vals) = multiply_row_fine_level(row_idx, &a, &b, &config);
                                black_box((cols, vals))
                            })
                        }
                    );
                ...
                if let Some(row_idx) = select_representative_row(&a, RowType::Large) {
                    group.bench_with_input(
                        ...
                        |bench, _| {
                            bench.iter(|| {
                                let (cols, vals) = multiply_row_coarse_level(row_idx, &a, &b, &config);
                                black_box((cols, vals))
                            })
                        }
                    );
...
fn load_matrix_market(path: &Path) -> Result<SparseMatrixCSR<f64>, String> {
    ...
    for line in lines {
        ...
        let row = values[0].parse::<usize>()
            .map_err(|_| "Invalid row index")? - 1; // 1-indexed to 0-indexed
        let col = values[1].parse::<usize>()
            .map_err(|_| "Invalid column index")? - 1; // 1-indexed to 0-indexed
        let val = values[2].parse::<f64>()
