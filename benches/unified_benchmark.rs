...
use magnus::test_tiers::{generate_test_matrices, BenchmarkTier, TestTier, TierConfig};
use magnus::{
...
/// Benchmark traditional matrix sizes
fn benchmark_traditional_sizes(
    group: &mut criterion::BenchmarkGroup<criterion::measurement::WallTime>,
    config: &TierConfig,
) {
...
fn benchmark_parallel_scaling(
    ...
    // Serial benchmark
    group.bench_function(BenchmarkId::new("scaling", "serial"), |b| {
        b.iter_batched(
            || {
    ...
    // Parallel benchmark
    group.bench_function(BenchmarkId::new("scaling", "parallel"), |b| {
        b.iter_batched(
            || {
                let a = generate_sparse_matrix(test_size, test_size, nnz_per_row);
                let b = generate_sparse_matrix(test_size, test_size, nnz_per_row);
    ...
    if config.matrix_sizes[0] <= 5000 {
        group.bench_function(BenchmarkId::new("scaling", "reference"), |b| {
            b.iter_batched(
                || {
                    let a = generate_sparse_matrix(test_size, test_size, nnz_per_row);
                    let b = generate_sparse_matrix(test_size, test_size, nnz_per_row);
