...
mod tests {
    ...
    fn test_csr_roundtrip() {
        ...
        for i in 0..original.n_rows {
            let mut original_row: Vec<_> =
                original.row_iter(i).map(|(col, &val)| (col, val)).collect();

            let mut roundtrip_row: Vec<_> = roundtrip
                .row_iter(i)
                .map(|(col, &val)| (col, val))
                .collect();
    ...
    fn test_csc_roundtrip() {
        ...
        for j in 0..original.n_cols {
            let mut original_col: Vec<_> =
                original.col_iter(j).map(|(row, &val)| (row, val)).collect();

            let mut roundtrip_col: Vec<_> = roundtrip
                .col_iter(j)
                .map(|(row, &val)| (row, val))
                .collect();
    ...
    fn test_sprs_multiply_via_conversion() {
        ...
        let a = SparseMatrixCSR::new(2, 2, vec![0, 2, 3], vec![0, 1, 1], vec![1.0f64, 2.0, 3.0]);
