...
/// # Safety
///
/// This macro uses unsafe NEON intrinsics. The following preconditions must be met:
/// - `$idx` must be a valid uint32x4_t NEON vector register
/// - `$val` must be a valid float32x4_t NEON vector register  
/// - `$lane_a` and `$lane_b` must be compile-time constants in the range [0, 3]
/// - The NEON vectors must be properly aligned and initialized before use
///
/// The macro maintains the invariant that corresponding elements in `$idx` and `$val`
/// remain paired after the swap operation.
...
fn scalar_sort_accumulate(col_indices: &[usize], values: &[f32]) -> (Vec<usize>, Vec<f32>) {
    ...
    let mut pairs: Vec<_> = col_indices
        .iter()
        .zip(values.iter())
        .map(|(&idx, &val)| (idx, val))
        .collect();
...
/// Sort exactly 4 elements using NEON SIMD instructions
///
/// # Safety
///
/// This function uses NEON intrinsics and requires:
/// - The target platform must support NEON (ARM with NEON feature)
/// - Input slices must have exactly 4 elements
/// - The function must be called in a context where NEON is available
...
#[target_feature(enable = "neon")]
/// Sort 5-8 elements using NEON with padding
///
/// # Safety
///
/// This function uses NEON intrinsics and requires:
/// - The target platform must support NEON
/// - Input slices must have between 5 and 8 elements
/// - Padding with u32::MAX/0.0 is safe as these values are filtered out
...
#[inline(always)]
/// Perform bitonic sort on 8 elements using NEON
///
/// # Safety
///
/// This function uses NEON intrinsics and requires:
...
/// - The target platform must support NEON
/// - Arrays must be properly aligned for NEON operations
...
unsafe fn neon_hybrid_sort(col_indices: &[usize], values: &[f32]) -> (Vec<usize>, Vec<f32>) {
    ...
    if num_chunks > 1 {
        let mut pairs: Vec<_> = all_indices
            .iter()
            .zip(all_values.iter())
            .map(|(&idx, &val)| (idx, val))
            .collect();
...
#[inline(always)]
/// Compare and conditionally swap two NEON vectors
///
/// # Safety
///
/// This function uses NEON intrinsics and requires:
/// - Valid NEON vector registers as inputs
/// - The target platform must support NEON
/// - Vectors must be properly initialized
...
fn merge_two_chunks(idx_in: &[u32], val_in: &[f32], idx_out: &mut [u32], val_out: &mut [f32]) {
