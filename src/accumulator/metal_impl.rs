...
impl MetalContext {
    fn new() -> Option<Self> {
        ...
        let bitonic_pipeline = device.new_compute_pipeline_state_with_function(&bitonic_function)
            .ok()?;
    ...
    fn gpu_bitonic_sort(&self, indices: &[u32], values: &[f32]) -> (Vec<u32>, Vec<f32>) {
        ...
        for stage in 0..num_stages {
            for pass in 0..=stage {
                ...
                encoder.set_bytes(2, mem::size_of::<u32>() as u64, &(1 << stage) as *const _ as *const _);
                encoder.set_bytes(3, mem::size_of::<u32>() as u64, &pass_of_stage as *const _ as *const _);
                encoder.set_bytes(4, mem::size_of::<u32>() as u64, &n_padded as *const _ as *const _);
                
                let threads_per_group = MTLSize::new(256, 1, 1);
                let thread_groups = MTLSize::new(
                    (n_padded as u64 + 255) / 256,
                    1,
                    1,
                );
        ...
        let sorted_indices = unsafe {
            std::slice::from_raw_parts(indices_ptr, n).to_vec()
        };
        let sorted_values = unsafe {
            std::slice::from_raw_parts(values_ptr, n).to_vec()
        };
...
mod tests {
    ...
    fn test_metal_sort() {
        if let Some(acc) = MetalAccumulator::new() {
            ...
            assert!(sorted_idx.len() >= 1000 && sorted_idx.len() <= 1001, 
                    "Expected ~1000 unique indices, got {}", sorted_idx.len());
        }
    }
}