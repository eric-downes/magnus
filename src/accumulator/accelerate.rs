...
impl SimdAccelerator<f32> for AccelerateAccumulator {
    fn sort_and_accumulate(&self, col_indices: &[usize], values: &[f32]) -> (Vec<usize>, Vec<f32>) {
        ...
        unsafe {
            accelerate_sort_and_accumulate(col_indices, values)
        }
    }
}

unsafe fn accelerate_sort_and_accumulate(col_indices: &[usize], values: &[f32]) -> (Vec<usize>, Vec<f32>) {
...
pub fn accelerate_sort_pairs(col_indices: &[usize], values: &[f32]) -> (Vec<usize>, Vec<f32>) {
    ...
    let mut pairs: Vec<(usize, f32)> = col_indices.iter()
        .zip(values.iter())
        .map(|(&idx, &val)| (idx, val))
        .collect();
