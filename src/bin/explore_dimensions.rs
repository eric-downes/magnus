...
use magnus::reduced_parameter_space::{
    ...
    ScalingAnalyzer, SmartPiSampler,
};
...
fn explore_pi_space() {
    ...
    println!("Total test matrices: {}", 
             configs.len() * configs[0].base_sizes.len() * 
    ...
    println!("Reduction from full space: {:.1}x", 
...
fn benchmark_critical() {
    ...
    for (i, pi_groups) in critical.iter().enumerate() {
        ...
        for &size in &config.base_sizes {
            ...
            println!("  n={:5}: {:.3}ms ({} nnz)", 
                     size, elapsed.as_millis(), a.nnz());
        }
...
fn test_scaling_laws() {
    ...
    println!("  π₁ = {:.2} (cache utilization)", pi_groups.pi1_cache_utilization);
    println!("  π₈ = {:.2} (NNZ ~ √n scaling)", pi_groups.pi8_nnz_distribution);
    println!();
    ...
    if times.len() >= 2 {
        ...
        for i in 1..times.len() {
            ...
            println!("  {}→{}: actual={:.2}x, expected={:.2}x, error={:.1}%",
                     sizes[i-1], sizes[i], time_ratio, expected_ratio, error);
        }
    }
}
...
fn run_full_exploration() {
    ...
    for (i, config) in configs.iter().take(test_subset).enumerate() {
        ...
        if i % 10 == 0 {
            ...
            println!("Progress: {:.1}% | Tested: {} | Rate: {:.1} tests/s | Result NNZ: {}",
                     progress, tested, rate, c.nnz());
        }
    }
