...
impl DimensionalConstants {
    ...
    pub fn categorize_units() -> Vec<(&'static str, PhysicalUnit, String)> {
        vec![
            ...
            ("AVX512_VECTOR_WIDTH_BYTES", PhysicalUnit::Bytes, 
             format!("{} B", AVX512_VECTOR_WIDTH_BYTES)),
            ("AVX2_VECTOR_WIDTH_BYTES", PhysicalUnit::Bytes,
             format!("{} B", AVX2_VECTOR_WIDTH_BYTES)),
            ("NEON_VECTOR_WIDTH_BYTES", PhysicalUnit::Bytes,
             format!("{} B", NEON_VECTOR_WIDTH_BYTES)),
            ("SCALAR_VECTOR_WIDTH_BYTES", PhysicalUnit::Bytes,
             format!("{} B", SCALAR_VECTOR_WIDTH_BYTES)),
            
            ("AVX512_OPTIMAL_CHUNK_SIZE", PhysicalUnit::Elements,
             format!("{} E", AVX512_OPTIMAL_CHUNK_SIZE)),
            ("AVX2_OPTIMAL_CHUNK_SIZE", PhysicalUnit::Elements,
             format!("{} E", AVX2_OPTIMAL_CHUNK_SIZE)),
            ("NEON_OPTIMAL_CHUNK_SIZE", PhysicalUnit::Elements,
             format!("{} E", NEON_OPTIMAL_CHUNK_SIZE)),
            ("GENERIC_OPTIMAL_CHUNK_SIZE", PhysicalUnit::Elements,
             format!("{} E", GENERIC_OPTIMAL_CHUNK_SIZE)),
            
            // ACCUMULATOR THRESHOLDS [Elements]
            ("AVX512_DENSE_THRESHOLD", PhysicalUnit::Elements,
             format!("{} E", AVX512_DENSE_THRESHOLD)),
            ("AVX2_DENSE_THRESHOLD", PhysicalUnit::Elements,
             format!("{} E", AVX2_DENSE_THRESHOLD)),
            ("NEON_DENSE_THRESHOLD", PhysicalUnit::Elements,
             format!("{} E", NEON_DENSE_THRESHOLD)),
            ("GENERIC_DENSE_THRESHOLD", PhysicalUnit::Elements,
             format!("{} E", GENERIC_DENSE_THRESHOLD)),
            
            ("METAL_GPU_THRESHOLD", PhysicalUnit::Elements,
             format!("{} E", METAL_GPU_THRESHOLD)),
            ("METAL_SIZE_THRESHOLD", PhysicalUnit::Elements,
             format!("{} E", METAL_SIZE_THRESHOLD)),
            
            // CAPACITY AND SIZE [Elements or Pure ratios]
            ("INITIAL_CAPACITY_DIVISOR", PhysicalUnit::Pure,
             format!("{} (ratio)", INITIAL_CAPACITY_DIVISOR)),
            ("MAX_SORT_ACCUMULATOR_CAPACITY", PhysicalUnit::Elements,
             format!("{} E", MAX_SORT_ACCUMULATOR_CAPACITY)),
            ("DEFAULT_SORT_ACCUMULATOR_SIZE", PhysicalUnit::Elements,
             format!("{} E", DEFAULT_SORT_ACCUMULATOR_SIZE)),
            
            // SIMD THRESHOLDS [Elements]
            ("AVX512_MIN_ELEMENTS", PhysicalUnit::Elements,
             format!("{} E", AVX512_MIN_ELEMENTS)),
            ("ACCELERATE_SIMD_THRESHOLD", PhysicalUnit::Elements,
             format!("{} E", ACCELERATE_SIMD_THRESHOLD)),
            ("NEON_MIN_ELEMENTS", PhysicalUnit::Elements,
             format!("{} E", NEON_MIN_ELEMENTS)),
            
            // MEMORY AND CACHE [Bytes]
            ("DEFAULT_CACHE_LINE_SIZE", PhysicalUnit::Bytes,
             format!("{} B", DEFAULT_CACHE_LINE_SIZE)),
            ("DEFAULT_L2_CACHE_SIZE", PhysicalUnit::Bytes,
             format!("{} B", DEFAULT_L2_CACHE_SIZE)),
            ("TEST_L2_CACHE_SIZE", PhysicalUnit::Bytes,
             format!("{} B", TEST_L2_CACHE_SIZE)),
            ("CONSERVATIVE_MEMORY_THRESHOLD", PhysicalUnit::Bytes,
             format!("{} B", CONSERVATIVE_MEMORY_THRESHOLD)),
            ("MODERATE_MEMORY_THRESHOLD", PhysicalUnit::Bytes,
             format!("{} B", MODERATE_MEMORY_THRESHOLD)),
            ("AGGRESSIVE_MEMORY_THRESHOLD", PhysicalUnit::Bytes,
             format!("{} B", AGGRESSIVE_MEMORY_THRESHOLD)),
            
            // PREFETCH [Pure or Elements]
            ("PREFETCH_DISTANCE_THRESHOLD", PhysicalUnit::Elements,
             format!("{} E", PREFETCH_DISTANCE_THRESHOLD)),
            ("PREFETCH_COUNT_MULTIPLIER", PhysicalUnit::Pure,
             format!("{} (ratio)", PREFETCH_COUNT_MULTIPLIER)),
            
            // HIT RATES [Pure - probabilities]
            ("HIGH_HIT_RATE_THRESHOLD", PhysicalUnit::Pure,
             format!("{}", HIGH_HIT_RATE_THRESHOLD)),
            ("MEDIUM_HIT_RATE_THRESHOLD", PhysicalUnit::Pure,
             format!("{}", MEDIUM_HIT_RATE_THRESHOLD)),
            
            // MATRIX PROPERTIES [Pure or Elements]
            ("SPARSE_DENSITY_THRESHOLD", PhysicalUnit::Pure,
             format!("{}", SPARSE_DENSITY_THRESHOLD)),
            ("B_MATRIX_AVG_NNZ_THRESHOLD", PhysicalUnit::Elements,
             format!("{} E/row", B_MATRIX_AVG_NNZ_THRESHOLD)),
            ("MIN_AVG_NNZ_THRESHOLD", PhysicalUnit::Elements,
             format!("{} E/row", MIN_AVG_NNZ_THRESHOLD)),
            
            // TOLERANCES [Pure - dimensionless]
            ("FLOAT_COMPARISON_EPSILON", PhysicalUnit::Pure,
             format!("{:e}", FLOAT_COMPARISON_EPSILON)),
            ("METAL_COMPUTATION_TOLERANCE", PhysicalUnit::Pure,
             format!("{:e}", METAL_COMPUTATION_TOLERANCE)),
            ("LARGE_COMPUTATION_RELATIVE_ERROR", PhysicalUnit::Pure,
             format!("{:e}", LARGE_COMPUTATION_RELATIVE_ERROR)),
            
            // DISPLAY [Elements - counts]
            ("MAX_DISPLAY_ROWS", PhysicalUnit::Elements,
             format!("{} rows", MAX_DISPLAY_ROWS)),
            ("MAX_DISPLAY_ELEMENTS_PER_ROW", PhysicalUnit::Elements,
...
impl ReducedParameterSpace {
    ...
    pub fn reconstruct_parameters(
        ...
        let avg_nnz_per_row = (pi_groups.pi8_nnz_distribution * (base_matrix_size as f64).sqrt()) as usize;
        ...
        let gpu_threshold = (pi_groups.pi5_gpu_utilization * base_matrix_size as f64 * density) as usize;
        
        // From π₁: chunk_size = π₁ * cache_size / element_size
        let chunk_size = (pi_groups.pi1_cache_utilization * base_cache_size as f64 / element_size as f64) as usize;
        ...
        let min_elements = (vector_width as f64 / (pi_groups.pi2_simd_efficiency * element_size as f64)) as usize;
        ...
        let accumulator_capacity = (pi_groups.pi7_accumulator_ratio * base_matrix_size as f64 * density) as usize;
