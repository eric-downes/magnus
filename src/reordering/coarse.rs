    ...
    T: Copy + Num + AddAssign,
{
    ...
    pub fn new(a: &SparseMatrixCSR<T>, coarse_rows: &[usize]) -> Self {
        ...
        let matrix = SparseMatrixCSC::new(a.n_rows, a.n_cols, col_ptr, row_idx, values);
...
impl CoarseLevelReordering {
        ...
        T: Copy + Num + AddAssign,
    {
        ...
        for i in 0..batch_size {
            ...
            let chunk_cols: Vec<usize> =
                chunk_accumulators[i].iter().map(|&(col, _)| col).collect();
            ...
            if results[i].1.is_empty() {
                ...
                let (merged_cols, merged_vals) =
                    merge_sorted_arrays(&results[i].1, &results[i].2, &chunk_cols, &chunk_vals);
    ...
    T: Copy + Num + AddAssign,
{
    ...
    for start_idx in (0..n_rows).step_by(reordering.batch_size) {
        ...
        let batch_results = reordering.process_batch(&a_hat_csc, b, start_idx, end_idx, config);
...
mod tests {
    ...
    fn test_a_hat_csc_construction() {
        ...
        let mut col0_pairs: Vec<(usize, f64)> = col0_indices
            .iter()
    ...
    fn test_empty_row() {
        ...
        let b = SparseMatrixCSR::new(2, 2, vec![0, 1, 2], vec![0, 1], vec![2.0f64, 3.0]);
    ...
    fn test_larger_matrix() {
        ...
        let a = SparseMatrixCSR::new(n, n, a_row_ptr, a_col_idx, a_values);
        ...
        let b = SparseMatrixCSR::new(n, n, b_row_ptr, b_col_idx, b_values);
