...
use super::{exclusive_scan, ChunkMetadata, Reordering};
...
impl FineLevelReordering {
    ...
    fn generate_intermediate_products<T>(
        &self,
        a_row: usize,
        a: &SparseMatrixCSR<T>,
        b: &SparseMatrixCSR<T>,
...
mod tests {
    use super::*;
    use crate::matrix::config::{Architecture, MagnusConfig, SystemParameters};
    ...
    fn test_multiply_row_fine_level() {
        ...
        let a = SparseMatrixCSR::new(1, 1, vec![0, 1], vec![0], vec![1.0f64]);

        let b = SparseMatrixCSR::new(1, 1, vec![0, 1], vec![0], vec![1.0f64]);
