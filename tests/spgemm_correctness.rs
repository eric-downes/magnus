...
use magnus::{SparseMatrixCSR, reference_spgemm, magnus_spgemm, MagnusConfig};
...
fn test_magnus_against_reference() {
    ...
    for i in 0..result_reference.n_rows {
        let ref_row: Vec<_> = result_reference.row_iter(i)
            .map(|(col, &val)| (col, val))
            .collect();
            
        let magnus_row: Vec<_> = result_magnus.row_iter(i)
            .map(|(col, &val)| (col, val))
            .collect();
