...
fn test_reference_vs_sprs() {
    ...
    for (ref_val, sprs_val) in result_ref.values.iter().zip(sprs_values.iter()) {
        assert!((ref_val - sprs_val).abs() < 1e-10, 
                "Value mismatch: {} vs {}", ref_val, sprs_val);
    }
}
