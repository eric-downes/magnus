...
use magnus::utils::{from_sprs_csc, from_sprs_csr, to_sprs_csc, to_sprs_csr};
...
fn test_csc_conversions() {
    ...
    for j in 0..csc.n_cols {
        let mut original_col: Vec<_> = csc.col_iter(j).map(|(row, &val)| (row, val)).collect();

        let mut roundtrip_col: Vec<_> = roundtrip_csc
            .col_iter(j)
            .map(|(row, &val)| (row, val))
            .collect();
