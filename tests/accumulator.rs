...
use magnus::{
    create_accumulator, multiply_row_dense, multiply_row_sort, Accumulator, SparseMatrixCSR,
};
...
fn test_accumulator_with_larger_matrices() {
    ...
    let a = SparseMatrixCSR::new(size, size, a_row_ptr, a_col_idx, a_values);
    ...
    let b = SparseMatrixCSR::new(size, size, b_row_ptr, b_col_idx, b_values);
...
fn test_sort_accumulator_with_complex_matrix() {
    ...
    for row in 0..4 {
        ...
        assert_eq!(
            cols_dense.len(),
            cols_sort.len(),
            "Row {} has different number of non-zeros",
            row
        );
        assert_eq!(
            cols_dense, cols_sort,
            "Row {} has different column indices",
            row
        );
