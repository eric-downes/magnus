...
use magnus::{SparseMatrixCSR, MagnusConfig, RowCategory};
...
fn create_banded_matrix(size: usize, bandwidth: usize) -> SparseMatrixCSR<f64> {
    ...
    for i in 0..size {
        ...
        let end_col = if i + bandwidth < size { i + bandwidth + 1 } else { size };
...
fn test_disable_coarse_level() {
    ...
    let a = SparseMatrixCSR::new(
        1, 5000,  // One row with many potential connections
        vec![0, 5000],
