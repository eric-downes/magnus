use magnus::{
    magnus_spgemm, multiply_row_coarse_level, multiply_row_fine_level, process_coarse_level_rows,
...
fn test_fine_vs_coarse_reordering() {
    ...
    let a = SparseMatrixCSR::new(1, 1, vec![0, 1], vec![0], vec![2.0f64]);

    let b = SparseMatrixCSR::new(1, 1, vec![0, 1], vec![0], vec![3.0f64]);
...
fn test_larger_matrices() {
    ...
    let a = SparseMatrixCSR::new(n, n, vec![0, 1, 2, 3], vec![0, 1, 2], vec![1.0, 1.0, 1.0]);

    // Create a diagonal matrix B
    let b = SparseMatrixCSR::new(n, n, vec![0, 1, 2, 3], vec![0, 1, 2], vec![2.0, 3.0, 4.0]);
